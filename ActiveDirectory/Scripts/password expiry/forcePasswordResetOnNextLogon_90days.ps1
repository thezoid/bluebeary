function writeLog([string]$message, [string]$status) {
    ...
    switch ($status.ToLower()) {
        "info" { write-host "*[$(get-date -Format "yyyyMMMdd@hh:mm:ss")] $($message)" -ForegroundColor Blue; try { "*[$(get-date -Format "yyyyMMMdd@hh:mm:ss")] $($message)" | Out-File "c:\temp\logs\$(get-date -Format "yyyyMMMdd")-90PwdEnforce" -Append }catch {} }
        "warning" { write-host "![$(get-date -Format "yyyyMMMdd@hh:mm:ss")] $($message)" -ForegroundColor Yellow; try { "![$(get-date -Format "yyyyMMMdd@hh:mm:ss")] $($message)" | Out-File "c:\temp\logs\$(get-date -Format "yyyyMMMdd")-90PwdEnforce" -Append }catch {} }
        "error" { write-host "!!![$(get-date -Format "yyyyMMMdd@hh:mm:ss")] $($message)" -ForegroundColor Red; try { "!!![$(get-date -Format "yyyyMMMdd@hh:mm:ss")] $($message)" | Out-File "c:\temp\logs\$(get-date -Format "yyyyMMMdd")-90PwdEnforce" -Append }catch {} }
    }
}
...
$knownServiceAccounts = get-content "$($PSScriptRoot)\knownServiceAccounts.txt"
$exceptionList = get-content "$($PSScriptRoot)\90dayPwdException.txt" # Get-ADGroupMember -Identity some90dExceptionGroup | select samaccountname
...
foreach ($user in $users) {
    if ($user.SamAccountName -like "svc*" -or $user.SamAccountName -like "srv*" -or $knownServiceAccounts -contains $user.SamAccountName -or $exceptionList -contains $user.SamAccountName) {
        #Write-Warning "Skipping $($user.SamAccountName) - it is a service account"
    ...
    if ($user.SamAccountName -like "a_*" -or $user.SamAccountName -like "d_*" -or $user.SamAccountName -like "w_*" -or $user.SamAccountName -like "s_*" -or $knownServiceAccounts -contains $user.SamAccountName -or $exceptionList -contains $user.SamAccountName) {
        #Write-Warning "Skipping $($user.SamAccountName) - it is an admin account"
    ...
    if ($user.PasswordLastSet) {
        if (((New-TimeSpan -Start $user.PasswordLastSet.tostring("yyyy-MM-dd") -End (get-date -Format "yyyy-MM-dd")).Days -ge 90)) {
            ...
            Set-ADUser -Identity $user.SamAccountName -CannotChangePassword:$false -PasswordNeverExpires:$false -ChangePasswordAtLogon:$true
        }
    }
    else {
