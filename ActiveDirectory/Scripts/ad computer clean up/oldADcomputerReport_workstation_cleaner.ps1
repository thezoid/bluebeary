...
if($fromFile){
    ...
    $targets = import-csv -path $inputFilePath -Header @("Name","LastLogonDate","Days Since LastLogonDate","Enabled","DistinguishedName","OperatingSystem","OperatingSystemVersion","SID") -Delimiter ","
}else{
    ...
    $targets = get-adcomputer -filter * -properties LastLogonDate,lastLogonTimestamp,enabled,OperatingSystem,OperatingSystemVersion `
        ...
        | ?{$_.distinguishedname -like "*OU=Workstation,DC=domain,DC=com" `
...
foreach($target in $targets){
    ...
    if($operation.tolower() -eq "move" -or $operation.tolower() -eq "m"){
        write-host "Disabling and moving $($target.name) to Purgatory"
